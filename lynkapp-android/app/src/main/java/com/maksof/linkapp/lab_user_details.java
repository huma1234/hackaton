         ...
         try{
          if(profile!=null) {
            ...
            try {
             ...
             try {
                 description = profile.get(0).getString("description");
             }catch (JSONException e){
                 e.printStackTrace();
             }
              try { address = profile.get(0).getString("address");}catch (JSONException e){
                  e.printStackTrace();
              }
                  try { webs = profile.get(0).getString("website");}catch (JSONException e){
                      e.printStackTrace();
                  }
                      try { latittude = profile.get(0).getString("latitude");}catch (JSONException e){
                          e.printStackTrace();
                      }
                          try { longitude = profile.get(0).getString("longitude");}catch (JSONException e){
                              e.printStackTrace();
                          }
                              try { isShared = profile.get(0).getString("isShared");
                                  if(isShared.equals("true")){
                                      profSharedByCrntUser = true;
                                  }
                                  else {
                                      profSharedByCrntUser = false;
                                  }
                              }catch (JSONException e){
                                  e.printStackTrace();
              ...
              Object recm = null;
              try {
                  recm = profile.get(0).get("recommendedBy");
                  LinkedTreeMap<Object, Object> t = (LinkedTreeMap) recm;
                  Object getrow1 = t.get("values");
                  recommendedBy = (ArrayList<JSONObject>) getrow1;
                  if(recommendedBy.size()>0){
                  Object getrow3 = recommendedBy.get(0);
                  ...
                  if(t4.get("name")!=null)
                      last_person_recommended.setText(t4.get("name").toString());
                    }}
              catch (JSONException e) {
                  try{
                  recommendedBy = new ArrayList<>();
                  recm = profile.get(0).get("addedBy");
                  LinkedTreeMap<Object, Object> t = (LinkedTreeMap) recm;
                  Object getrow4 = t.get("nameValuePairs");
                  LinkedTreeMap<Object, Object> t4 = (LinkedTreeMap) getrow4;
                  last_person_recommended.setText(t4.get("first_name").toString() + " " + t4.get("last_name").toString());}
                  catch (ClassCastException ce){
                      try{

                          last_person_recommended.setText((String)recm);
                      }catch (Exception ex){
                          last_person_recommended.setText("");
                      }
                  }
                  catch (Exception e3){last_person_recommended.setText("");}
              }
              try {
                  Object getrow = profile.get(0).get("reviews");
                  LinkedTreeMap<Object, Object> t = (LinkedTreeMap) getrow;
                  Object getrow1 = t.get("values");
                  listOfAddedBy = (ArrayList<JSONObject>) getrow1;

              for (int j = 0; j < listOfAddedBy.size(); j++) {
                  Object getrow3 = listOfAddedBy.get(j);
                  LinkedTreeMap<Object, Object> t3 = (LinkedTreeMap) getrow3;
                  Object getrow4 = t3.get("nameValuePairs");
                  LinkedTreeMap<Object, Object> t4 = (LinkedTreeMap) getrow4;
                  rating += Float.parseFloat(t4.get("rating").toString());
              }
              avgRating = rating / listOfAddedBy.size();
              }catch (JSONException e){
...
//        Modal With List Function
        openSharingUserModal.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                UR_Adapter ur_adapter = new UR_Adapter(recommendedBy);
                ...
                post.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        try {
                            ...
                            if(postFeedBack.getText().toString().equals("")||postFeedBack.getText().toString()==null){
                                postFeedBack.setError("Review Required");
                                flag = false;
                            }
                            if(flag==true) {
                                addReviewsObject.put("contactId", contactId);
                                addReviewsObject.put("rating", ratingBar.getRating());
                                addReviewsObject.put("feedback", postFeedBack.getText().toString());
                                if(newImg == true)
                                    addReviewsObject.put("imageUrl", labimg);
                                else
                                    addReviewsObject.put("imageUrl", "");

//                                commonClass.apiIntegration(lab_user_details.this, "review/addreview", addReviewsObject.toString(), PreferenceUtils.getToken(lab_user_details.this), "postReviews");
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
...
//    User Reviews Adapter
    ...
    public static class ReviewAdapter1 extends BaseAdapter{
        ...
        public Object getItem(int position) {
            try {
                return this.review.get(position);
            } catch (JSONException e) {
                e.printStackTrace();
            }
            return null;
...
//                                commonClass.apiIntegration(activity, "review/deletereview", jsonObject.toString(), PreferenceUtils.getToken(activity),"deleteFeedback");
            ...
            try {
                if(PreferenceUtils.getKeyId(activity).equals(review.getString("recommendedBy")))
                    ...
                    nameTxt.setText(review.getString("fullName"));
                String reg="[a-zA-Z0-9]";
                if((Character.toUpperCase(Character.toUpperCase(review.getString("fullName").toString().charAt(0))) + "").matches(reg))
                    mainLetter.setText(Character.toUpperCase(review.getString("fullName").toString().charAt(0)) + "");
                else mainLetter.setText(Constants.firstChar);

                    reviewDesc.setText(review.getString("feedback").toString());
                    ratingBar.setRating(Float.parseFloat(review.getString("rating").toString()));
...
//            commonClass.apiIntegration(activity, "review/getreviewbycontactid", getRev.toString(), PreferenceUtils.getToken(activity), "getReviews");
    ...
    public void unshareOrDelete(String what){
        ...
        if(profSharedByCrntUser){
            confirmationText.setText("Are you sure you want to stop sharing? This will remove the profile from your Lynks.");
        }else{
